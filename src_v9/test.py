...
def infer(low_img, normal_img, opt, generator, refiner):
    if isinstance(low_img, str):
        low_img_path = low_img
    else:
        low_img_path = 'usr/local/low_light_img/data/*.png'
    ...
    with torch.no_grad():
        ...
        output_filename = os.path.join('output_images',os.path.basename(low_img_path).replace('.png','_enhanced.png'))
        save_image(pred_normal_image_rgb, output_filename)
        
        gray_filename = os.path.join('gray_images',os.path.basename(low_img_path).replace('.png','_grayscale.png'))
        save_image(pred_normal_image_gray, gray_filename)
        
        output_filename_G = os.path.join('G_images',os.path.basename(low_img_path).replace('.png','_G_img.png'))
        save_image(pred_refined_map, output_filename_G) 
