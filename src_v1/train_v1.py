...
def train_fn(generator, refiner, train_dataloader, opt_gen, opt_ref, gen_loss, ref_loss, train_writer, epoch,  opt):
    ...
    for batch_idx, (low_image_rgb, normal_image_rgb, normal_image_gray) in enumerate(loop):
        ...
        train_writer.add_scalar("Generator loss", total_loss, global_step=epoch+1)
        train_writer.add_scalar("Refiner loss", r_loss, global_step=epoch+1)
        train_writer.add_scalar("PSNR", curr_psnr, global_step=epoch+1)
...
def main():
    ...
    for epoch in range(opt["train"]["epochs"]):
        ...
        train_psnr = train_fn(generator, refiner, train_dataloader, opt_gen, opt_ref, gen_loss, ref_loss, train_writer, epoch,  opt)
