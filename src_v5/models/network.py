...
class Refiner(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.conv_1x1_63 = nn.Conv2d(6, 3, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, coarsemap, gray):
        ...
        half_input_map = torch.cat([resized_256, half_diff_image], dim = 1)
        #half_input_map = self.conv_1x1_63(half_input_map)
        quater_input_map = torch.cat([resized_128, quater_diff_image], dim = 1)
        #quater_input_map = self.conv_1x1_63(quater_input_map)
    
        origin_map = self.residual6_encoder(coarsemap)
        half_map = self.residual5_encoder(half_input_map)
        quater_map =self.residual4_encoder(quater_input_map)
