...
def train_fn(generator, refiner, train_dataloader, opt_gen, opt_ref, gen_loss, ref_loss, train_writer, opt, loss_history):
    ...
    loss_history['train_psnr'].append(avg_psnr)

    return avg_psnr, epoch_loss

def val_fn(generator, refiner, val_dataloader, val_writer, opt, loss_history):
    ...
    loss_history['val_psnr'].append(avg_psnr)
...
def main():
    ...
    loss_history = {'train_loss': [], 'train_psnr': [], 'val_psnr': []}
    ...
    for epoch in range(opt["train"]["epochs"]):
        ...
        train_psnr, train_loss = train_fn(generator, refiner, train_dataloader, opt_gen, opt_ref, gen_loss, ref_loss, train_writer, opt, loss_history)

        loss_history['train_loss'].append(train_loss)
        loss_history['train_psnr'].append(train_psnr)

        test_psnr = val_fn(generator, refiner, val_dataloader, val_writer, opt, loss_history)
        logger.info(f'[Info] Training PSNR: {train_psnr}')
        logger.info(f'[Info] Testing PSNR: {test_psnr}')

        loss_history['val_psnr'].append(test_psnr)
        ...
        with open('loss_history_dilation.pkl', 'wb') as f:
