...
class RDN(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, growth_rate, num_blocks, num_layers):
        ...
        self.residual_dense_blocks = nn.ModuleList(
            [RDB(num_features, growth_rate, num_layers) for _ in range(num_blocks)]
        )
        ...
        self.conv3 = nn.Conv2d(num_features, out_channels, kernel_size=3, padding=1)

    def forward(self, x):
        sfe1 = self.conv1(x)
        sfe2 = self.conv2(sfe1)

        out = sfe2
        local_features = []
        for i in range(len(self.residual_dense_blocks)):
            out = self.residual_dense_blocks[i](out)
            local_features.append(out)

        out = torch.cat(local_features, 1)
        out = self.global_feature_fusion(out)
        out = self.conv3(out + sfe1)  # Global residual learning
        return out